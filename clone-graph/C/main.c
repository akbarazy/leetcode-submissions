...
struct Node* original[MAX_SIZE];
struct Node* cloned[MAX_SIZE];
int mapSize = 0;

struct Node* findClone(struct Node* node) {
...
void addMapping(struct Node* node, struct Node* clone) {
    original[mapSize] = node;
    cloned[mapSize] = clone;
    mapSize++;
}
...
struct Node* cloneGraph(struct Node* node) {
    ...
    struct Node* existing = findClone(node);
    if (existing != NULL)
        return existing;

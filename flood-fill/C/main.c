...
void dfs(int **image, int sr, int sc, int rows, int cols, int color, int newColor) {
    if(sr < 0 || sc < 0 || sr >= rows || sc >= cols) return;
    if(image[sr][sc] != color) return;

    image[sr][sc] = newColor;

    dfs(image, sr + 1, sc, rows, cols, color, newColor);
    dfs(image, sr - 1, sc, rows, cols, color, newColor);
    dfs(image, sr, sc + 1, rows, cols, color, newColor);
    dfs(image, sr, sc - 1, rows, cols, color, newColor);
}

int **floodFill(
    int **image, int imageSize, int *imageColSize,
